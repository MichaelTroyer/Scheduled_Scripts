...
LONG_ARCHIVE with the same name, and then removes the uncompressed version from SHORT_BACKUP.
The tool then copies and datestamps the current BLM_Cultural_Resources.gdb to SHORT_BACKUP.
"""
...
ROOT = r'T:\CO\GIS\giswork\rgfo\development\cultural\BLM_Cultural_Resources'
...
CRM_GDB_PATH = os.path.join(ROOT, 'BLM_Cultural_Resources.gdb')
...
try:
    ...
    print "Created archive: {}".format(backup_to_archive)
    ...
    print"Removed backup: {}".format(last_months_path)
